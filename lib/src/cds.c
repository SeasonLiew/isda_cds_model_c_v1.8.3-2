 ...
 TBoolean        protectStart)
{
    ...
    {
        dl = JpmcdsDateListMakeRegular (startDate, endDate, dateInterval, stubType);// NOTE: only this execute, out put are date grid spaced by 3m interval from start date to end date
    }
    if (dl == NULL)
        goto done;
